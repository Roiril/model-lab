...
def build_asm(P, C, exports_dir, name, boards):
    ...
    f = (-mc, -mc)                                     # 5 本目
    a_legs = [(-co, 0.0), (-co - P.SPAN, 0.0)]
    b_legs = [(0.0, -co), (0.0, -co - P.SPAN)]
    r_pipe = P.PIPE_OD / 2
    parts = []
    ...
    parts.append(place(load_stl(exports_dir, "joint_a", "pipe_joint_28.stl"), 90, -mc, -J.LEG_X, h_center))
    # B: レールは世界 x。平らな面を角（-x）へ。ローカル +X → 世界 +x（回さない）
    parts.append(place(load_stl(exports_dir, "joint_b", "pipe_joint_28.stl"), 0, -J.LEG_X, -mc, h_center))
    ...
    wall = -J.LEG_X + J.X_TOP + ASM_RAIL_EXT
