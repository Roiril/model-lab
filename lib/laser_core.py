...
# 小さいほど凸凹が多く細かくなる（見た目かわいい）。数はここで、嵌合の保持力は KERF で調整。
FINGER_TARGET = 11.0  # mm → ~56-68mm 辺で 5-7 本（細かめ＝かわいい見た目）
...
def living_hinge(u0, u1, v0, v1, col_space=3.0, seg_len=10.0, ligament=3.5,
    ...
    margin=0 が標準（スリットが端まで開口し、交互パターンで端も曲がる）。
    margin>0 は端に連続レールが残り、そのレールは曲がらないので注意
    （半径 R に曲げるとき表面ひずみ t/2R が材料の破断ひずみを超えると割れる/曲がらない）。
