...
水密検査では頂点・面の存在も確認する。空メッシュに対する `all(...)` を合格としない。
EXACTが入力の結合を空やごく少数の面へ崩す場合は、工程ごとの形状を調べる。
入力が閉じた非ゼロ体積であることを確かめた上で、該当工程だけ別solverを試せる。
切替後は閉じ方・寸法・干渉を再検査する。形状を変えて演算の不具合を隠さない。
