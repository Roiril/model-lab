...
ときだけ 350 x 320）。どのモードでも刷れるよう 300 x 300 に収める。
    全幅 = CORNER_OFF + SPAN + 2 * EDGE_R = 74.7 + 160 + 60 = 294.7（縁まで 2.65）
外側の縁は軸から EDGE_R = 30 なので、そこにあるソケット 4 本は pipe-foot の台座
（半径 34 の段・半径 26 の円錐）を付けず、根元の壁を厚くした筒にする（pipe-foot-corner の
初期版と同じ形）。5 本目は板の内側の角にあるので、まわりの板は半径 PLATE_R = 42 のまま。
...
上から見た配置（すべて mm。角の節点が原点、A の脚は x 軸上の負側、B の脚は y 軸上の負側）:
    ...
    角の節点 (0,0) は板の外（A1-B1 のあいだの縁は対角線 x + y = -32.3）
...
EDGE_R = 30.0                    # 脚 4 本まわりの半径（外側の縁）
BED = 300.0                      # H2D の 2 ノズル時の X。ここに入れば全モードで刷れる
EXTENT = CORNER_OFF + SPAN + 2 * EDGE_R          # 294.7 全幅 = 奥行き
assert EXTENT <= BED, f"造形板に入らない: {EXTENT}"

# --- ソケットの根元（脚 4 本。縁まで 30 しか無いので台座を付けず、根元の壁を厚くする）---
