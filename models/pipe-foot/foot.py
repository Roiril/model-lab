...
from params import (
    ...
    FLANGE_T, DISC_R, LOBE_R, BOLT_R, SCREW_D, SCREW_CHAMFER, CONE_BASE_R,
...
def boss_profile():
    """フランジ上面 → ソケット口元までの回転体プロファイル。

    台座は smoothstep で絞る。最大傾斜は 1.5*(28-18.3)/25 = 0.58 → 垂直から 30 度で、
    上へ細るだけなので無支持。口元はさらに TIP_R まで落として段差を小さくする。
    """
    pts = [(FLANGE_T, CONE_BASE_R)]
    for i in range(1, CONE_SEG + 1):
        t = i / CONE_SEG
        z = FLANGE_T + (SEAT_Z - FLANGE_T) * t
...
def build_foot(col_name="foot"):
    ...
    # フランジ：中央の円 ＋ 4 方向のふくらみ
    body = cyl("pipe_foot", DISC_R, Z_BUILD_BOT, FLANGE_T, col)
    ...
    # 台座＋ソケット
    boolean(body, revolve("boss", boss_profile(), col), "UNION")
