...
- 裏のひだ: 細い縦の溝 2本（`FLUTE_N`）を回転体カッターの boolean で彫る。上端はテーパー＋
  小さな丸めで「しゅっと」した先端に、下端は底面まで到達（底面は平ら）。マーカーには被らない
...
- `FLUTE_N` / `FLUTE_SPREAD` / `FLUTE_TOP` / `FLUTE_R` / `FLUTE_TAPER` / `FLUTE_TIP_R` — 裏ひだの本数・位置・太さ・先端の丸め
