...
- 高さ **50mm**（実測）、幅 ≈ 24mm、断面は楕円で奥行き = 幅 × 0.70（側面は正面より薄い）
...
- 裏のひだ: 縦の半円溝 4本（`FLUTE_N`）を boolean で彫る。裾は球カッターでなみなみに切り欠き
...
- `EYE_*` — 目の位置・大きさ・深さ
- `MARK_*` — マーカーの位置・径
- `FLUTE_N` / `FLUTE_TOP` / `FLUTE_R` / `HEM_SCALLOP_*` — 裏ひだと裾の波

## 既知の制限・TODO

- 裏のひだ・なみなみ裾は実物のドレープを様式化した近似（写真からの厳密復元ではない）
- なみなみ裾の外周に細かいギザつきが残る（表示スケールでは目立たない）
